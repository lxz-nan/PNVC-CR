...
class GaussianEncoder(AEHelper):
    ...
    @staticmethod
    def get_gaussian_prob(values, scales, interval=None):
        ...
        if interval is not None:
            ...
            interval = 0.5
        upper = _standardized_cumulative((interval - values) / scales)
        lower = _standardized_cumulative((-interval - values) / scales)
        prob = upper - lower
        prob = LowerBound.apply(prob, 1e-9)
        return prob
    ...
    def get_laplace_prob(values, scales):
        ...
        upper = _cdf((values + 0.5) / scales)
        lower = _cdf((values - 0.5) / scales)
        prob = upper - lower
        prob = LowerBound.apply(prob, 1e-9)
        return prob
