...
def get_state_dict_st3(ckpt_path):
    ckpt = torch.load(ckpt_path, map_location=torch.device('cpu'))
    if "video_net" in ckpt:
        video_net = ckpt['video_net']
        consume_prefix_in_state_dict_if_present(video_net, prefix="module.")
    else:
        video_net = None
    if "injector" in ckpt:
        injector = ckpt['injector']
        consume_prefix_in_state_dict_if_present(injector, prefix="module.")
    else:
        injector = None
    return video_net, injector
